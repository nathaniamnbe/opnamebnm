    ...
    >
      <div className="card" style={{ width: "100%", borderRadius: 0 }}>
        ...
        <div style={{ marginTop: "20px", marginBottom: "0px" }}>
          <a
            href={`https://instruksi-lapangan.vercel.app/il/index.html?auth=true&role=${encodeURIComponent(user?.role || "userRole")}&email=${encodeURIComponent(user?.email || "userEmail")}&cabang=${encodeURIComponent(user?.cabang || "userCabang")}`}
            target="_blank"
            rel="noopener noreferrer"
            className="btn"
            style={{
